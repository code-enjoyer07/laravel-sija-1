...
<main class="p-6 bg-gray-50 rounded-lg shadow-sm mt-8 max-w-xl mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-8">Edit Penerbit</h1>

    <form action="{{ route('admin.penerbit.update', $penerbit->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="penerbit_nama" class="block text-gray-700 font-medium mb-2">Nama <span class="text-red-500">*</span></label>
            <input id="penerbit_nama" type="text" name="penerbit_nama"
                   value="{{ old('penerbit_nama', $penerbit->penerbit_nama) }}" required
        ...
        <div>
            ...
            <textarea id="penerbit_alamat" name="penerbit_alamat" rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded-md resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('penerbit_alamat', $penerbit->penerbit_alamat) }}</textarea>
        ...
        <div>
            ...
            <input id="penerbit_notelp" type="text" name="penerbit_notelp"
                   value="{{ old('penerbit_notelp', $penerbit->penerbit_notelp) }}"
        ...
        <div>
            ...
            <input id="penerbit_email" type="email" name="penerbit_email"
                   value="{{ old('penerbit_email', $penerbit->penerbit_email) }}"
